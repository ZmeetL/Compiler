...
memory_type ir_func_test(p_ir_func p_func, memory_stack **global_stack, memory_stack **top_stack) {
    assert(p_func);
    p_list_head p_node;
    ...
    p_list_head p_list = ir_func_get_basic_block_entry(p_func);
    memory_type re = ir_basic_block_test(p_list, &(*global_stack), &(*top_stack));
    stack_pop(&(*top_stack));
    return re;
}