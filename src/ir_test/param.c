...
void ir_param_list_test(p_ir_param_list p_param_list, memory_stack *global_stack, memory_stack *top_stack) {
    assert(p_param_list);
    int id = 0;
    p_list_head p_node;
    list_for_each(p_node, &p_param_list->param) {
        ...
        p_ir_operand p_operand = p_param->p_param;
        ...
        case temp_var:
            if (p_operand->p_temp_sym->is_pointer) {
                memory_type p_arry = ir_operand_data_get(p_operand, global_stack, top_stack->prev);
                if (p_arry.type < 0)
                ...
                top_stack->mem[top_stack->mem_cnt++] = ir_operand_data_get(p_operand, global_stack, top_stack->prev);
            break;
        case declared_var:
